...
export const deleteProduct = async (
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRaw`DELETE FROM "Review" WHERE "productId" = ${id}`;
      
      // Second, delete all cart items with this product
      await tx.$executeRaw`DELETE FROM "CartItem" WHERE "productId" = ${id}`;
      
      // Note: We don't delete OrderItems as they represent historical purchase data
